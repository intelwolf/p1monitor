...
def Main():
    ...
    if process_status == True: 
        ...
        flog.info("Stop van programma, succes.")
    else:
        flog.info("Stop van programma, gefaald.")
...
def updateHistory(record):
    ...
    time_offset = 3600
    if is_dst(): #summer time compensation. 
       time_offset+=3600	
    ...
    insertWeatherHistory(rec_select, record['city_id'], record['city'],\
    ...
    insertWeatherHistory(rec_select_day, record['city_id'], record['city'],\
...
def setFileFlags():
	setFile2user(const.FILE_DB_WEER_FILENAME,'p1mon')
	...
	setFile2user(const.DIR_FILEDISK+tail,'p1mon')
	setFile2user(logfilename,'p1mon')
...
def deleteFromHistoryDb():
	...
	sql_del_str = "delete from "+const.DB_WEATHER_UUR_TAB+" where timestamp <  '"+str(datetime.strptime(timestr,"%Y-%m-%d %H:%M:%S") - timedelta(days=1096))+"'"
	try:
	...
	sql_del_str = "delete from "+const.DB_WEATHER_DAG_TAB+" where timestamp <  '"+str(datetime.strptime(timestr,"%Y-%m-%d %H:%M:%S") - timedelta(days=1096))+"'"
	try:
		flog.debug(inspect.stack()[0][3]+": sql delete="+sql_del_str)
...
if __name__ == "__main__":
    ...
    try:
        flog = fileLogger(logfilename,prgname)    
