...
def main(argv):
    ...
    parser.add_argument('-ri', '--readindex',
        required=False,
        help="lees waarde van het status item aan de hand van de opgeven index." )
    
    parser.add_argument('-wi', '--writeindex',
        required=False,
        help="schrijf de waarde van het status item aan de hand van de opgeven index, is afhankelijk van writevalue." )
    
    parser.add_argument('-wv', '--writevalue',
        required=False,
        help="schrijf de waarde van het status item aan de hand van de opgeven index, is afhankelijk van writeindex." )
    ...
    if args.readindex != None:
        try:
            # index, flog
            _id, value, _label = rt_status_db.strget( str(args.readindex), flog )
            print ("#", str(value) )
            sys.exit(0)
        except Exception as e:
    ...
    if args.writeindex != None and args.writevalue != None:
        try:
            # value, index, flog
            rt_status_db.strset( str(args.writevalue), str(args.writeindex) , flog )
            sys.exit(0)
        except Exception as e:
