...
var GselectText      = ["6 "+months_text,"1 "+year_text,"2 "+year_text,"5 "+year_text] // #PARAMETER
...
function readJsonApiHistoryPowerMonth( cnt, db_index ){ 

    $.getScript( "/api/v1/powerproductionsolar/month/1/" + db_index+ "?limit=" + cnt, function( data, textStatus, jqxhr ) {
      try {
        var jsondata = JSON.parse(data); 
        ...
        readJsonApiWeatherHistoryMonth( maxrecords )
    ...
    $.getScript( "/api/v1/configuration/140", function( data, textStatus, jqxhr ) {
      try {
        ...
        for ( var i=0;  i<db_indeces_list.length; i++ ) {
            ...
            readJsonApiHistoryPowerMonth( cnt , db_indeces_list[i] );
        }
                ...
                series :{
                    ...
                    events: {
                        legendItemClick: function (event) {
                            ...
                            if  ( this.index === 0 ) {
                                toLocalStorage('powerprod-api-m-high-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 1 ) {
                                toLocalStorage('powerprod-api-m-low-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 2 ) {
                                toLocalStorage('powerprod-api-m-netto-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 3 ) {
                                toLocalStorage('powerprod-api-m-temp-visible',!this.visible);  // #PARAMETER
                            }
                        }
                    }
                },
                    ...
                    if(typeof(e.rangeSelectorButton)!== 'undefined') {
                        for (var j = 0;  j < GselectText.length; j++){    
                            if ( GselectText[j] == e.rangeSelectorButton.text ) {
                                toLocalStorage('powerprod-api-m-select-index',j+1); // PARAMETER
                                break;
                            }
                        }
                    }
...
<div class="top-wrapper-2">
    <div class="content-wrapper pad-13">
       <!-- header 2 -->
       <?php pageclock(); ?>
       <?php page_menu_header_api_powerproduction( 3 ); ?>
       <?php weather_info(); ?>
    </div>
</div>
...
<div class="mid-section">
    ...
    <!-- links -->
        <div class="frame-2-top">
            <span class="text-2"><?php echo strIdx(145);?></span>
        </div>
