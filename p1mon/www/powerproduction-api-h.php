...
var Gselected        = 0;
var GselectText      = ["12 "+hour_text,"1 "+day_text,"3 "+days_text,"5 "+days_text] // #PARAMETER
...
function readJsonApiHistoryPowerHour( cnt, db_index ){ 

    //console.log( db_index ) 
    $.getScript( "/api/v1/powerproductionsolar/hour/1/" + db_index+ "?limit=" + cnt, function( data, textStatus, jqxhr ) {
      try {
        var jsondata    = JSON.parse(data); 
    ...
    $.getScript( "/api/v1/configuration/140", function( data, textStatus, jqxhr ) {
      try {
        ...
        for ( var i=0;  i<db_indeces_list.length; i++ ) {
            ...
            readJsonApiHistoryPowerHour( cnt , db_indeces_list[i] );
        }
                ...
                series :{
                    ...
                    events: {
                        legendItemClick: function (event) {
                            ...
                            if  ( this.index === 0 ) {
                                toLocalStorage('powerprod-api-h-high-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 1 ) {
                                toLocalStorage('powerprod-api-h-low-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 2 ) {
                                toLocalStorage('powerprod-api-h-netto-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 3 ) {
                                toLocalStorage('powerprod-api-h-temp-visible',!this.visible);  // #PARAMETER
                            }
                        }
                    }
                },
                    ...
                    if(typeof(e.rangeSelectorButton)!== 'undefined') {
                        for (var j = 0;  j < GselectText.length; j++){    
                            if ( GselectText[j] == e.rangeSelectorButton.text ) {
                                toLocalStorage('powerprod-api-h-select-index',j+1); // PARAMETER
                                break;
                            }
                        }
                    }
...
$(function() {
    ...
    Gselected = parseInt(getLocalStorage('powerprod-api-h-select-index'),10);
    GseriesVisibilty[0] =JSON.parse(getLocalStorage('powerprod-api-h-high-tariff-visible'));  // #PARAMETER
...
<div class="top-wrapper-2">
    <div class="content-wrapper pad-13">
       <!-- header 2 -->
       <?php pageclock(); ?>
       <?php page_menu_header_api_powerproduction( 1 ); ?>
       <?php weather_info(); ?>
    </div>
</div>
...
<div class="mid-section">
    ...
    <!-- links -->
        <div class="frame-2-top">
            <span class="text-2"><?php echo strIdx(141);?></span>
        </div>
