...
var Gselected        = 0;
var GselectText      = ["1 "+week_text,"14 "+days_text,"1 "+month_text,"2 "+months_text] // #PARAMETER
...
function readJsonApiHistoryPowerDay( cnt, db_index ){ 

    $.getScript( "/api/v1/powerproductionsolar/day/1/" + db_index+ "?limit=" + cnt, function( data, textStatus, jqxhr ) {
      try {
        var jsondata = JSON.parse(data); 
        recordsLoaded           = jsondata.length;
        
        /*
        for ( var sites = jsondata.length; sites>0; sites-- ) {
            //set based
            for ( idx = jsondata[ sites-1 ][1].length-1; idx>-1; idx-- ) {
    ...
    $.getScript( "/api/v1/configuration/140", function( data, textStatus, jqxhr ) {
      try {
        ...
        for ( var i=0;  i<db_indeces_list.length; i++ ) {
            ...
            readJsonApiHistoryPowerDay( cnt , db_indeces_list[i] );
        }
                ...
                series :{
                    ...
                    events: {
                        legendItemClick: function (event) {
                            ...
                            if  ( this.index === 0 ) {
                                toLocalStorage('powerprod-api-d-high-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 1 ) {
                                toLocalStorage('powerprod-api-d-low-tariff-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 2 ) {
                                toLocalStorage('powerprod-api-d-netto-visible',!this.visible);  // #PARAMETER
                            }
                            if  ( this.index === 3 ) {
                                toLocalStorage('powerprod-api-d-temp-visible',!this.visible);  // #PARAMETER
                            }
                        }
                    }
                },
...
$(function() {
    ...
    GseriesVisibilty[0] =JSON.parse(getLocalStorage('powerprod-api-d-high-tariff-visible'));  // #PARAMETER
...
<div class="top-wrapper-2">
    <div class="content-wrapper pad-13">
       <!-- header 2 -->
       <?php pageclock(); ?>
       <?php page_menu_header_api_powerproduction( 2 ); ?>
       <?php weather_info(); ?>
    </div>
</div>
...
<div class="mid-section">
    ...
    <!-- links -->
        <div class="frame-2-top">
            <span class="text-2"><?php echo strIdx(143);?></span>
        </div>
