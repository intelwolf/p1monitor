...
include '/p1mon/www/util/page_menu_header_heating.php';
...
function readJsonIndoorTemperatureMonth( cnt ){ 
    $.getScript( "./api/v1/indoor/temperature/month?limit=" + cnt , function( data, textStatus, jqxhr ) {
      try {
       ...
       if(typeof(e.rangeSelectorButton)!== 'undefined') {
         for (var j = 0;  j < GselectText.length; j++){    
           if ( GselectText[j] == e.rangeSelectorButton.text ) {
             toLocalStorage('verwarming-m-select-temperatuur-index',j); // #PARAMETER
             break;
           }
         }
       }
      ...
      series: {
        ...
        events: {
          legendItemClick: function () {
            ...
            if ( this.index === 0 ) {
              toLocalStorage('verwarming-m-in-visible',!this.visible); // #PARAMETER
            }
            if ( this.index === 1 ) {
              toLocalStorage('verwarming-m-in-gem-visible',!this.visible); // #PARAMETER
            }
            if ( this.index === 2 ) {
              toLocalStorage('verwarming-m-uit-visible',!this.visible); // #PARAMETER
            }
            if ( this.index === 3 ) {
              toLocalStorage('verwarming-m-uit-gem-visible',!this.visible); // #PARAMETER
            }
          }
        }
      }  
...
$(function() {
    ...
    GseriesVisibilty[0] = JSON.parse(getLocalStorage('verwarming-m-in-visible'));
    GseriesVisibilty[1] = JSON.parse(getLocalStorage('verwarming-m-in-gem-visible'));
...
<div class="top-wrapper-2">
    <div class="content-wrapper pad-13">
       <!-- header 2 -->
        <?php pageclock(); ?>
        <?php page_menu_header_heating(4); ?>  <!-- parameter -->
        <?php weather_info(); ?>
    </div>
</div>
